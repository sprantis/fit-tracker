...
function nutritionSearch(e) {
    ...
    // local variable to dynamically grab inner HTML of food button when function is called
    ...
    .then(function(response) {
        ...
        // note that not all foods at foods[0] have the same order of nutrients. Need to add in checks later.
...
// clear out localstorage where the key is nutritionProgress andre-display the nutrition progress table
function clearNutritionStatHistory (){
    ...
    localStorage.setItem("nutritionProgress", JSON.stringify(nutritionStorageArray));
    showNutritionProgress();
}

// needed to use FOR loop, because muscleGroupBtns returns a node list instead of a single node. Can only use .addEventListerner to a single node, not a node list.
...
submitNutritionBtn.addEventListener('click', function (e) {
    ...
    // get localstorage array needed to define array locally within function. Global variable would not work
    const nutritionProgressArray = getNutritionProgress();

    // Note: First attempts to check for valid integer to float value for protein, carbs, fat, calories, servingSize, and servings failed. Need to investigate. push a new object to localstorage array that has current user
    //inputs when this function is called
    if(
        (parseInt(protein.value) || parseInt(protein.value) == 0) &&
        (parseInt(carbs.value) || parseInt(carbs.value) == 0) &&
        (parseInt(fat.value) || parseInt(fat.value) == 0) &&
        (parseInt(calories.value) || parseInt(calories.value) == 0) &&
        (parseInt(servingSize.value) || parseInt(servingSize.value) == 0) &&
        (parseInt(servings.value) || parseInt(servings.value) == 0)
    ){
        ...
        errorModalText.innerHTML = 'Error: Please enter in valid number values for protein, carbohydrates, fat, calories, servings size, and servings.';
        errorModal.classList.add('is-active');
    }

    localStorage.setItem("nutritionProgress", JSON.stringify(nutritionProgressArray));
